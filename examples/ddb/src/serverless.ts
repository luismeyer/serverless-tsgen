  ...
  type QueryItemsOptions = Omit<DocumentClient.QueryInput, "TableName" | "IndexName">;
    ...
    export async function getQzmkrTable<T>(id: string,options?: GetItemOptions): Promise<DDBResult<T>>
   {
    ...
    export async function queryQzmkrTableNameBirthIndex<T>(birth: number,name?: string,options?: QueryItemsOptions): Promise<DDBResult<T[]>>
   {
    ...
    ...(name ? { ":name": name } : {})
        ...
        KeyConditionExpression: "#birth = :birth"  + ( name ? " and #name = :name" : "" )
      })
      .promise();
    ...
    export async function queryQzmkrTableNameIndex<T>(name: string,options?: QueryItemsOptions): Promise<DDBResult<T[]>>
   {
